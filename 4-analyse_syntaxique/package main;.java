...
//		q3();
	...
	// Réponse à la question 1
		public static void q1() {
			...
			try {
				...
				Query q2 = readFileQuery("queries/relation.sparql");

				System.out.println("===== Relations =====");
				// Exécute la query
				runQuery(q2, model);
				
				Query q3 = readFileQuery("queries/attribut.sparql");
	...
	public static void q2() {
		...
		try {
			...
			Query q3 = readFileQuery("queries/CNRS.sparql");

			System.out.println("===== Titre avec CNRS =====");
			// Exécute la query
			runQuery(q3, model);
			
			Query q4 = readFileQuery("queries/rubriques2011.sparql");

			System.out.println("===== Rubriques des bulletins de 2011 =====");
			// Exécute la query
			runQuery(q4, model);
			//il manque "Horizon Formation" et "Horizon Formation Enseignement"
			
			Query q5 = readFileQuery("queries/images.sparql");
			
			System.out.println("===== Numéros des bulletins 3 imgs + légendes =====");
			// Exécute la query
			runQuery(q5, model);
	...
	// Réponse à la question 3
		public static void q3() {
			...
			try {
				...
				Query q2 = readFileQuery("queries/rubriquespopulaires.sparql");

				System.out.println("===== Rubriques populaires =====");
				// Exécute la query
				runQuery(q2, model);
				//"Evenement" "Actualité Innovation" et "Focus" sont les rubrique les plus fréquentes
		...
		public static void q4() {
			...
			try {
				...
				Query q2 = readFileQuery("queries/capteursIR.sparql");

				System.out.println("===== Capteurs non-infrarouges et pas internet =====");
				// Exécute la query sur le modèle en utilisant l'index
				 queryIndex(q2,wholeIndex,model);
			
				 Query q3 = readFileQuery("queries/creation.sparql");

					System.out.println("===== Bulletin traitant de création =====");
					// Exécute la query sur le modèle en utilisant l'index
					 queryIndex(q3,wholeIndex,model);
					 

				Query q4 = readFileQuery("queries/creationChaire.sparql");

					System.out.println("===== Création d'une chaire =====");
					// Exécute la query sur le modèle en utilisant l'index
